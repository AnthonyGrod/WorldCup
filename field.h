...
class Field {
    ...
    public:
        ...
        virtual void on_landing([[maybe_unused]] std::shared_ptr<Player> player) {}

        virtual void through_field([[maybe_unused]] std::shared_ptr<Player> player) {}
};
...
class Match_Field : public Field {
    ...
    public:
        explicit Match_Field(const std::string &field_name, double weight)
            : Field(std::move(field_name)),
              weight(weight) {
                cumulation = 0;
        }
