...
public:
    ...
    // jezeli gracz staje na danym polu
    virtual void on_landing(std::shared_ptr<Player> player) = 0;

    // jezeli gracz przechodzi przez dane pole
    virtual void through_field(std::shared_ptr<Player> player) = 0;
};
...
public:
    explicit Match_Field(const std::string &field_name, double weight, zdzislaw_t rate)
            : Field(std::move(field_name)),
              weight(weight),
              rate(rate) {
        cumulation = 0;
